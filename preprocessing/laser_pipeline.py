...
# ── Pfade ─────────────────────────────────────────────────────────────────
HERE   = Path(__file__).parent       # Verzeichnis dieser Datei (= preprocessing)
...
def main():
    import argparse
    global OUTPUT
    ap = argparse.ArgumentParser(description="Laserlinie in Referenzframe(s) detektieren + fitten.")
    ap.add_argument("bilder", nargs="*", help="einzelne TIF-Dateien; leer = alle raw/*.tif")
    ap.add_argument("--out", default=None, help="Ausgabeordner (Default: ./output)")
    a = ap.parse_args()
    if a.out:
        OUTPUT = Path(a.out)                            # Ausgabeordner ueberschreibbar (fuer Pipeline)
    OUTPUT.mkdir(parents=True, exist_ok=True)
    tifs = [Path(b) for b in a.bilder] if a.bilder else sorted(RAW.glob("*.tif"))  # CLI-Bilder oder alle raw/
