...
SRC = {
    ...
    "cn":  pfade.canny_json(),
    "un":  os.path.join(BASE, "unet", "data.json"),
    "sam": os.path.join(BASE, "sam", "data.json"),
}
...
print(f"-> {OUT}/viewer.html ({n} Frames, 4 Methoden)")
...
<style>
 ...
 .fd{color:#38d15b}.cn{color:#2ec7d6}.un{color:#ff5555}.sam{color:#e05cff} .hint{font-size:12px;color:#777}
</style></head><body>
<h1>4-Methoden-Vergleich &mdash; Serie 260402-174444</h1>
<p class="sub">Frame-Diff <span class="fd">&#9632;</span> Canny <span class="cn">&#9632;</span> U-Net <span class="un">&#9632;</span> SAM <span class="sam">&#9632;</span>
...
<div class="wrap">
 ...
 <div class="panel"><canvas id="plot" width="600" height="560"></canvas>
  <div class="info">max Dicke: FD <span id="vfd" class="fd">0</span> CN <span id="vcn" class="cn">0</span> UN <span id="vun" class="un">0</span> SAM <span id="vsam" class="sam">0</span> <span id="einh">px</span></div></div>
</div>
...
const KEYS=['fd','cn','un','sam'], COL={fd:'#38d15b',cn:'#2ec7d6',un:'#ff3030',sam:'#e05cff'};
...
function drawPlot(){
 ...
 const f=F[cur], vmax={fd:0,cn:0,un:0,sam:0};
...
['tfd','tcn','tun','tsam','smooth'].forEach(id=>document.getElementById(id).onchange=render);
